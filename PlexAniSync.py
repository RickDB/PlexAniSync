...
def plex_get_watched_episodes_for_show_season(
    ...
    if not os.path.isfile(mapping_file):
        ...
        for line in file:
            try:
                ...
                mapping = anilist_custom_mapping(
                    series_title, season, anime_id)
                    ...
                    if hasattr(mediaCollection, 'completed'):
                        for media_item in mediaCollection.completed:
                            if media_item is not None:
                                series_obj = anilist_mediaitem_to_object(
                                    media_item)
...
def anilist_search_item_to_obj(item):
    try:
        if(item):
            return anilist_single_mediaitem_to_object(item.data)
    except BaseException:
        pass
    return None
...
def anilist_single_mediaitem_to_object(media_item):
    id = media_item.Media.id
    sType = ''
    sFormat = ''
    source = ''
            ...
            if not all(matched_anilist_series) or not matched_anilist_series:
                ...
                if custom_mapping_id > 0:
                    ...
                    logger.warning(
                        ...
                        (plex_title))
                    media_id_search = anilist_find_id_best_match(
...
def match_anilist_series_with_seasons(
        anilist_series,
        plex_series_all,
        plex_title,
        plex_year,
    ...
    while counter_season <= plex_total_seasons:
        ...
        if(counter_season == 1):
            ...
            if(custom_mapping_id > 0):
                ...
                media_id_search = anilist_find_id_season_best_match(
...
def anilist_add_by_id(
        anilist_id,
        plex_title,
        plex_year,
        plex_watched_episode_count):
    matched_anilist_series = []
    media_lookup_result = anilist_search_by_id(anilist_id)
    if(media_lookup_result):
        anilist_obj = anilist_search_item_to_obj(media_lookup_result)
        if(anilist_obj):
